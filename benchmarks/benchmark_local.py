...
def run_benchmark():
    ...
    print("=" * 60)
    print("CHUNKED ZS-ISAB vs VANILLA TABPFN BENCHMARK")
    print("=" * 60)
    
    row_counts = [1024, 4096, 16384, 65536, 131072, 262144, 524288]
    
    for model_name in ['Vanilla TabPFN', 'ZS-ISAB']:
        print(f"\n--- Testing {model_name} ---")
        
        for N in row_counts:
            # We don't run Vanilla past 16K because it takes too long and crashes anyway
            if model_name == 'Vanilla TabPFN' and N > 65536:
                print(f"Skipping N = {N:,} for Vanilla (would OOM or take hours).")
                continue
