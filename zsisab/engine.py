...
def _zsisab_forward_impl(self, original_forward_fn, src: torch.Tensor, src_mask, src_key_padding_mask, num_prototypes: int, chunk_size: int, verbose: bool = False) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        if isinstance(src_mask, int):
            ...
            # Multi-layer diverse prototype seeds for depth-wise ensemble coverage
            layer_seed = 42 + int(abs(W_q[0, 0].item() * 10000)) % 10007
            ...
            for i in range(0, seq_len, chunk_size):
                ...
                # Delete intermediate tensors explicitly just to be safe
                del src_norm_chunk, Q_chunk, scores, attn_weights, attn_out, src_norm2, mlp_out

            concat_dim = 1 if is_batch_first else 0
            src_out = torch.cat(output_chunks, dim=concat_dim)
